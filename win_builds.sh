...
mkdir -p bin/Win64
mkdir -p bin/Win32
declare -a plugins=("SimplePlugin")
...
(
    cd bin
    rm -f "SimplePlugin-Win64-${VERSION}.zip"
    rm -f "SimplePlugin-Win32-${VERSION}.zip"
    zip -r "SimplePlugin-Win64-${VERSION}.zip" Win64
    zip -r "SimplePlugin-Win32-${VERSION}.zip" Win32
)
