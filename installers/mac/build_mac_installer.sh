...
script_file=SimplePlugin.pkgproj
...
pkg_dir=SimplePlugin_Installer_Packaged
rm -Rf $pkg_dir
mkdir $pkg_dir
productsign -s "$TEAM_ID" ../../build/SimplePlugin.pkg $pkg_dir/SimplePlugin-signed.pkg

echo "Notarizing installer package..."
INSTALLER_PASS=$(more ~/Developer/mac_installer_pass)
npx notarize-cli --file $pkg_dir/SimplePlugin-signed.pkg --bundle-id com.chowdsp.SimplePlugin --asc-provider "$TEAM_ID" --username chowdsp@gmail.com --password "$INSTALLER_PASS"

echo "Building disk image..."
vol_name=Install_SimplePlugin-$app_version
rm -f "$vol_name"
